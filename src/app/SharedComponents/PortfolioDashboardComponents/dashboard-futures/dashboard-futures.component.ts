...
@Component({
  selector: 'app-dashboard-futures',
  templateUrl: './dashboard-futures.component.html',
  styleUrls: ['./dashboard-futures.component.scss']
})
export class DashboardFuturesComponent implements OnInit, OnDestroy {
  ...
  isAllPromiseResolved:boolean=false;
...
this.googleSheetAPIServiceRef.setSheetName('Spot_Trades')
    ...
    switch (this.exchangeName) {
      case 'Mexc': {
        ...
        this.getOpenOrdersRealtime(this.exchangeName)
        ...
        Promise.all([
          ...
          this.functionsServiceRef.loadAllSheetData_WithoutMatTable(this.exchangeName, 'Futures_Transfer', 500, this.componentDestroyed$),
          this.functionsServiceRef.loadSheetData(this.exchangeName, this.sheetName, '', 1, this, this.componentDestroyed$),


        ]).then(([accountResponse, sheetData_transferResponse, sheetData_SheetName]) => {
          ...
          //#region Handle transfer response
          //       if (transferResponse) {
          ...
          console.log('openPositionData : ', this.openPositionData)
  ...
  executeAfterPromiseMethods(exchangeName: string) {
    ...
    this.isAllPromiseResolved=true;
    this.generateCalendar();
    if (exchangeName != 'Ourbit') {
      this.render_AccountBalance();
    }
    this.render_PortfolioPnl();
  ...
  getOldestMonthAndYear(data: any[]): string {
    ...
    const sortedData = data.map(item => ({
      ...item,
      Close_Date: new Date(item.Close_Date)
    })).sort((a, b) => a.Close_Date - b.Close_Date);

    // Get the oldest date
    const oldestDate = sortedData[0].Close_Date;
  ...
  calculateTransferInfo() {
    ...
    const seedMoney = (parseFloat(totalInAmount) - parseFloat(totalOutAmount)).toFixed(3);
    // const seedMoneyPercentage = ((parseFloat(seedMoney) / parseFloat(totalInAmount)) * 100).toFixed(3);
    const pnlTrades = this.columnTotals.Pnl_USDT

    const ClaimedBonus = this.futuresTransferInfo
      .filter((element: any) => element.Type === 'IN_BONUS')
      .reduce((acc: number, element: any) => acc + parseFloat(element.Amount), 0)
      .toFixed(3);
    this.ClaimedBonus = parseFloat(ClaimedBonus);

    this.calculatedTransfersInfo = [
      {
        totalInAmount: parseFloat(totalInAmount),
        totalOutAmount: parseFloat(totalOutAmount),
        totalBonusAmount: ClaimedBonus,
        // seedMoney: parseFloat(seedMoney),
        // seedMoneyPercentage: parseFloat(seedMoneyPercentage),
  ...
  getOpenOrdersRealtime(exchangeName: string) {
    console.log(exchangeName)



    if (exchangeName.toUpperCase() === 'MEXC') {
      this.mexcAPIServiceRef.getOpenPositionsMexcFutures().then(async (response) => { // Mark the callback as async
        if (response) {
          this.openPositionData = response;

          const openPositionData_temp = await Promise.all(response.map(async (element: any) => { // Use Promise.all to wait for all promises to resolve
            const formatDate = (timestamp: string) => {
              const myDate = new Date(parseInt(timestamp));
              return `${myDate.getFullYear()}-${(myDate.getMonth() + 1).toString().padStart(2, '0')}-${myDate.getDate().toString().padStart(2, '0')} ${myDate.getHours().toString().padStart(2, '0')}:${myDate.getMinutes().toString().padStart(2, '0')}:${myDate.getSeconds().toString().padStart(2, '0')}`;
            }
            ...
            const fairPriceResponse = await this.mexcAPIServiceRef.getPriceBySymbol(element.symbol);
            const fairPrice = fairPriceResponse; // Adjust this if the fairPrice is not in response directly
            ...
            return {
              ...
              openType: element.openType == 1 ? 'Market' : element.openType == 2 ? 'Limit' : 'error changing openType',
              symbol: element.symbol.replace('_USDT', ''),  // Update the symbol
              holdVol: this.dataServiceRef.getStaticSymbolContractValue(element.holdVol, element.symbol),
              fairPrice: fairPrice // Now we correctly set the resolved fairPrice
            };
          }));

          this.openPositionData = openPositionData_temp.sort((a: any, b: any) => {
            const timeA = new Date(a.updateTime).getTime();
            const timeB = new Date(b.updateTime).getTime();
            return timeB - timeA;
          });
        } else {
          this.openPositionData = [];
  ...
  getOpenPositionPnl(item: any): any {
    let pnlObj: any = {};

    // Calculate the total cost (cost basis)
    const totalCost = item.holdAvgPrice * item.holdVol;

    // Calculate the profit and loss (PnL)
    let pnl;
    if ((item.positionType).toUpperCase() === 'BUY') {
      pnl = ((item.fairPrice * item.holdVol) - totalCost) + item.realised;
    }
    if ((item.positionType).toUpperCase() === 'SELL') {
      pnl = (totalCost - (item.fairPrice * item.holdVol)) + item.realised;
    }

    // Calculate the PnL percentage
    const pnlPercentage = (pnl / totalCost) * 100 * item.leverage

    // Return the PnL and PnL percentage
    return pnlObj = { pnl: pnl, pnlPercentage: pnlPercentage };
  }

   getPercentageUptoLiqudation(item: any): number {
    let percentageDifference: number = 0;

    if (item.positionType === 'BUY') {
        percentageDifference = ((item.fairPrice - item.liquidatePrice) / item.liquidatePrice) * 100;
    } else if (item.positionType === 'SELL') {
        percentageDifference = ((item.liquidatePrice - item.fairPrice) / item.liquidatePrice) * 100;
    }

    return percentageDifference*item.leverage;
}
...
onOpenChartAnalysisDialog(item: any) {
  const dialogRef = this._dialog.open(ChartAnalysisComponent, {
    data: { exchangeName: (this.exchangeName != 'Ourbit' ? this.exchangeName :'Binance').toUpperCase(), tradingPair: item.symbol,location:'dashboard' },
    disableClose: false,
    minWidth: '95vw',
    minHeight: '93vh',
    maxHeight: '93vh'
  ...
  calculateColumnTotals() {
    ...
    this.sheetData.forEach((element: any) => {
      ...
      if (element.Pnl_USDT > 0) {
        this.columnTotals.influencerPNL[influencer].totalProfit += element.Pnl_USDT;
      } else if (element.Pnl_USDT < 0) {
        this.columnTotals.influencerPNL[influencer].totalLoss += element.Pnl_USDT;
      }
  ...
  render_PortfolioPnl() {
    ...
    // const oldestDate = new Date(this.portfolioPnlInfo[0].oldestDate).getTime(); // Convert oldestDate to timestamp
    // const latestDate = new Date(this.portfolioPnlInfo[0].latestDate).getTime(); // Convert latestDate to timestamp
    const seedMoney = parseFloat(this.portfolioPnlInfo[0].seedMoney);
    const portfolioPnlCalculated = parseFloat(this.portfolioPnlInfo[0].portfolioPnlCalculated);

    // Calculate percentage difference
    const percentageDifference = parseFloat((((portfolioPnlCalculated - seedMoney) / Math.abs(seedMoney)) * 100).toFixed(3));

    // Calculate amount difference
    const amountDifference = parseFloat((portfolioPnlCalculated - seedMoney).toFixed(3));
      ...
      series: [
        {
          ...
          data: [
            {
              name: `<span style="color: ${seedMoney < 0 ? '#fa4b42' : '#2caffe'}">Seed Money</span>`,
              y: seedMoney,
              drilldown: 'remain',
              color: seedMoney < 0 ? '#fa4b42' : '#2caffe'
            },
            {
              name: `<span style="color: ${seedMoney < portfolioPnlCalculated ? '#50B498' : '#fa4b42'}">Acc. Balance</span>`,
              y: portfolioPnlCalculated,
              drilldown: 'portfolioPnlCalculated',
              color: seedMoney < portfolioPnlCalculated ? '#50B498' : '#fa4b42'
            },
            {
              name: `<span style="color: ${amountDifference < 0 ? '#fa4b42' : '#00e272'}">Pnl</span>`,
              y: amountDifference,
              drilldown: 'Pnl',
              color: amountDifference < 0 ? '#fa4b42' : '#00e272'
            },
            {
              name: `<span style="color: ${percentageDifference < 0 ? '#fa4b42' : '#00e272'}">Percentage</span>`,
  ...
  render_InfluencerPNL() {
    ...
    this.chartOptions_InfluencerPNL = {
      chart: {
        ...
        // spacingBottom: 50, // Adjust bottom spacing to ensure the button is not overlapped
        // spacingTop: 50, // Adjust bottom spacing to ensure the button is not overlapped
      ...
      xAxis: {
        categories: sortedInfluencers.map(name => name.substring(0, 9) + '..'), // Display only the first 9 characters + '..'
        title: {
          text: 'Influencers'
        },
        labels: {
      ...
      yAxis: {
        // Calculate min and max values with additional space
        min: Math.min(...seriesData.map(d => d.y)) - 10, // Add extra space below the lowest value
        max: Math.max(...seriesData.map(d => d.y)) + 10, // Add extra space above the highest value
      ...
      series: [{
        type: 'bar', // Ensure the series type is bar
  ...
  render_InfluencersTotalTrades() {
    ...
    let seriesData = uniqueInfluencers.map(influencer => {
      ...
      const totalBuyTrades = influencerTrades.filter(trade => trade.Direction === 'BUY').length;
      const totalSellTrades = influencerTrades.filter(trade => trade.Direction === 'SELL').length;
      const totalProfitableTrades = influencerTrades.filter(trade => trade.Pnl_USDT > 0).length;
      const totalLostTrades = influencerTrades.filter(trade => trade.Pnl_USDT < 0).length;
    ...
    this.chartOptions_InfluencersTotalTrades = {
      ...
      yAxis: {
        min: 0,
        max: Math.max(...seriesData.map(d => d.y)) + 15, // Add extra space above the highest value
        title: {
          text: 'Trades',
          margin: 20 // Add margin to the y-axis title
  ...
  render_Transfers() {
    ...
    const pieData = [
      ...
      { name: 'OUT', y: Math.abs(totalOutAmount), color: '#50B498' }
    ];
